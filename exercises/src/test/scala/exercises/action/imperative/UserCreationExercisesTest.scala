...
class UserCreationExercisesTest extends AnyFunSuite with ScalaCheckDrivenPropertyChecks {
  ...
  test("parseInputToBoolean parse input to Boolean") {
    assert(parseYesNo("Y"))
    assert(!parseYesNo("N"))
  }

  def invalidYesNoInputGen: Gen[String] =
    arbitrary[String].filter(word => word != "Y" && word != "N")

  test("All invalid inputs should throw an exception") {
    forAll(invalidYesNoInputGen) { (input: String) =>
      assertThrows[IllegalArgumentException](parseYesNo(input))
    }
  }
  ...
  test("readDateOfBirth accepts valid format") {
    forAll { (inputDate: LocalDate) =>
      val formattedInput: String = dateOfBirthFormatter.format(inputDate)
      val console = Console.mock(ListBuffer(formattedInput), ListBuffer())
  ...
  test("readUser example") {
    forAll { (instant: Instant) =>
      val inputs = ListBuffer("Eda", "dd-mm-yyyy", "18-03-2001", "yes", "Y")
      val outputs = ListBuffer.empty[String]
      val console = Console.mock(inputs, outputs)
      val result = readUser(console, Clock.constant(instant))
