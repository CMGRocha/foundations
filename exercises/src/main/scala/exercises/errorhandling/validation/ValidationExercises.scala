...
object ValidationExercises {
  ...
  def validateCountry(countryCode: String): Validation[FormError, Country] =
    ...
    else Invalid(NEL.one(InvalidFormat(countryCode)))
  ...
  def checkUsernameCharacters(username: String): Validation[InvalidCharacters, Unit] =
    username.toList.filterNot(isValidUsernameCharacter) match {
      case Nil          => Valid(())
      case invalidChars => Invalid(NEL(InvalidCharacters(NEL(invalidChars.head, invalidChars.tail))))
    }
...
//      .zip(checkUsernameCharacters(username))
//      .map(_ => Username(username))
  ...
  def validateUser(usernameStr: String, countryStr: String): Validation[FormError, User] =
    (validateUsername(usernameStr), validateCountry(countryStr)).zipWith(User)
  ...
  def validateUserImproved(usernameStr: String, countryStr: String): Validation[FieldError, User] =
    (
  ...
  def field[A](fieldId: String, validation: Validation[FormError, A]): Validation[FieldError, A] =
    validation.mapErrorAll(formError => NEL(FieldError(fieldId, formError)))
//    validation match {
//      case Invalid(fromError) => FieldError(fieldId, fromError).invalid
